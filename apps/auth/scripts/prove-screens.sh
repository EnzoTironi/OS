...
[[ "$status" == "200" ]] || fail "GET /onboard/probe-token status ${status}"
grep -q "Google is not planted" "$body" || fail "GET /onboard/probe-token missing Google is not planted"
...
[[ "$status" == "200" ]] || fail "GET /onboard/done status ${status}"
grep -q "Volta pra conversa" "$body" || fail "GET /onboard/done missing Volta pra conversa"
