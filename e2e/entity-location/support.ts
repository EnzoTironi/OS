...
export const actionId = "inventory.assignLocation";
export const activationActionId = "zoen.definition.activate";
export const locationEntityId = "inventory.location.wh-1";
export const resourceId = "inventory.position.widget-pro.wh-1";
export const tenantA = "tenant.a";
export const validAt = new Date("2026-08-21T12:00:00.000Z");
const postgresPortFallback = 55_516;
const zoendPortFallback = 58_681;
const keycloakPortFallback = 58_680;
...
export async function compileInventory(): Promise<CompiledDefinition> {
        ...
        policies: [
          {
            ...
            policyId: "policy.assignLocation.r1",
...
export function assignLocationRequest(
  ...
  return {
    ...
    inputs: [entityInput("location", location)],
    operationId: `operation.assignLocation.${suffix}`,
    proposalId: `proposal.assignLocation.${suffix}`,
    resourceId,
...
export function composeOutput(...arguments_: string[]): Promise<string> {
  return command("docker", [
    "compose",
    "--project-name",
    "zoen-entity-location",
    "--file",
    path.join("e2e", "entity-location", "compose.yaml"),
    ...arguments_,
  ]);
}
...
export function queryLocation(
  ...
  return client.semanticQuery({
    ...
    selection: create(QuerySelectionSchema, {
      value: { case: "relationId", value: "inventory.location" },
    }),
    tenantId: tenantA,
    validAt: timestampFromDate(validAt),
  });
...
export function recordLocationText(
  client: WorldClient,
  definition: DefinitionReference,
  claimId: string,
) {
  return client.recordEvidence({
    claim: create(EvidenceClaimSchema, {
      ...
      provenance: create(EvidenceProvenanceSchema, {
        ...
        sourceId: "source.entity-location",
        sourceRef: `urn:zoen:entity-location:${claimId}`,
      }),
      ...
      value: exactValue({ case: "textValue", value: "warehouse-1" }),
    }),
    tenantId: tenantA,
  });
}
