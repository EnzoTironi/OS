...
export ZOEN_COMPANY_NAMESPACE="${application_namespace}"
third_party_images=(
...
tenants:
  ...
  - id: tenant.b
    ...
    nodePort: ${ZOEN_E2E_HARNESS_B_PORT}
web:
  ...
  - role: control-plane
    extraPortMappings:
      ...
      - containerPort: ${ZOEN_E2E_HARNESS_A_PORT}
        ...
        listenAddress: "127.0.0.1"
      - containerPort: ${ZOEN_E2E_HARNESS_B_PORT}
        hostPort: ${ZOEN_E2E_HARNESS_B_PORT}
        listenAddress: "127.0.0.1"
...
install_application() {
  ...
  helm upgrade --install zoen \
    ...
    --values "${profile_values}" --values "${overlay_values}" \
    --values "${generated_directory}/ports.yaml" \
    --set-file "policyManifest=${generated_directory}/policies.json" \
    "${artifact_flags[@]}"
}
...
if [[ ! -f dist/e2e/v1-company.js || ! -f dist/e2e/v1-company/prepare.js ]]; then
  echo "missing compiled v1-company runner; run \`just build\`" >&2
  exit 1
fi
...
definition_digest="$(node dist/e2e/v1-company/prepare.js)"
artifact_flags=(
...
write_ports_values
node e2e/v1-company/prepare-realm.mjs
...
node dist/e2e/v1-company.js
pass "v1-company" "mini-company cycle completed" "${recovery_file}"
trap - EXIT
cleanup_cluster
