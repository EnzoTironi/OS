...
const actor = {
  membership: "membership.cursor.primary",
  principal: "principal.cursor.reader",
} as const;
const alternateMembership = {
  membership: "membership.cursor.alternate",
  principal: actor.principal,
} as const;
const alternatePrincipal = {
  membership: "membership.cursor.other-principal",
...
const release = await plantBudgetRelease({
  budgets,
  databaseUrl,
  generatedDirectory,
  world,
...
const grants = [
  `${actor.principal}:${actor.membership}`,
  `${alternateMembership.principal}:${alternateMembership.membership}`,
  `${alternatePrincipal.principal}:${alternatePrincipal.membership}`,
];
for (const index of [1, 2, 3, 4, 5, 6, 7]) {
  const planted = plantObject(objectType, `patient.${index}`, grants);
  assert.equal(planted.status, 0, planted.stderr || planted.stdout);
}
const note = plantObject(alternateObjectType, "note.1", grants);
...
record(
  "typed_authority_slots_are_explicitly_absent",
  firstPage.body?.authorityEvaluation === "NOT_EVALUATED" &&
    firstClaims?.trustedAuthorityDigest === null &&
...
const artifactPath = await writeScenarioArtifact(repositoryRoot, scenario, {
  ...
  claims: {
    ...
    trustedAuthorityDigest: "NOT_EVALUATED",
  },
  dimensions: {
    basis:
    ...
    scope:
      "cursor-security substrate only; caller-supplied principal/membership authority and computation are explicitly outside this proof",
  },
  finalGates: {
    "FIN-05": {
      reason:
        "No authenticated Membership authority cut or denied-field/series/count/lineage/citation path is evaluated by this substrate journey.",
  ...
  journeys: {
    J4: {
      reason:
        "TrustedAuthorityDigest and authority cut are typed cursor slots but remain absent until the Membership authority repair lands.",
      verdict: "NOT_EVALUATED",
    },
  },
  release: {
