...
Production images and Helm must execute `dist/packages/effect-worker/src/worker.js`. `e2e/effects/worker.ts` is a thin import for scenario runners only. CI rejects `dist/e2e` in `deploy/helm`, `deploy/images`, and `deploy/scripts`.
...
`POST /channels/whatsapp/inbound` uses Standard Webhooks HMAC (timestamp skew, replay id, constant-time compare). Missing secret is 503; bad signature is 401. Durable replay rows are namespaced (`zoend:` / `gateway:`) so the proxy hop and the gateway can both persist without colliding. Identity `/identity/admin/*` requires a Bearer token: OIDC or `ZOEN_IDENTITY_ADMIN_TOKEN`. Fail closed. WhatsApp subjects are person E.164 or `s.whatsapp.net`/`c.us` JIDs; door JID and groups are rejected. Multiple active memberships without a tenant hint fail closed. Media is not ingested and must not be advertised as native.
