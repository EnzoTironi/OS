...
The effect runtime is private to the same VM: the HTTP connector binds `127.0.0.1:8081`, the Restate handler binds `127.0.0.1:9081`, and its exact-registration probe binds `127.0.0.1:9082`. None is a Fly service. The image bakes the Git commit into Restate service metadata; the registrar refuses URI, service type, handler, owner, or artifact drift and never force-registers it.
