...
DECLARE
    ...
    v_record jsonb;
    v_effect jsonb;
    v_conn text := 'effect_' || replace(cast(pg_backend_pid() as text), '-', '');
    v_op_insert text;
    v_effect_insert text;
BEGIN
    ...
    PERFORM dblink_exec(v_conn, v_op_insert);

    FOR v_effect IN SELECT value FROM jsonb_array_elements(v_effects)
    LOOP
        v_effect_insert := format(
            'INSERT INTO effect_request (namespace, operation_id, request_id, effect_definition_ref, intent_digest, payload, commit_revision) VALUES (%L, %L, %L, %L, %L, %L::jsonb, %s)',
            v_namespace,
            v_operation_id,
            v_effect->>'request_id',
            v_effect->>'effect_definition_ref',
            v_effect->>'intent_digest',
            (v_effect->'payload')::text,
            v_commit_revision
        );
        PERFORM dblink_exec(v_conn, v_effect_insert);
    END LOOP;
