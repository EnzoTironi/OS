...
trusted identifier/rule, crosswalk, or governed review
      │
      ▼
QUALIFY STATEMENTS / RELATIONS FOR SCOPED USE
authority rule says what may drive which projection/query/Action
      │
      ▼
OPERATIONAL PROJECTIONS / QUERIES / ACTION INPUTS
consumer declares which statement/relation kind + assurance is sufficient
```

At every arrow, prior evidence remains inspectable while retention/security policy permits it. Reprocessing may produce a **new proposal**; it does not rewrite what an earlier model/source/reviewer concluded.

Two corrections from the adversarial self-review are central:

```text
relation meaning != assurance != Action admissibility
operational qualification != canonical truth election
```
...
Likewise, saying a source statement is qualified to drive a specific projection or Action does **not** mean it became the metaphysically true value. Rival evidence can remain preserved and queryable when audit/reconciliation requires it.
...
6. **Operational qualification is scoped use, not truth election.** A statement may be allowed to drive one projection/Action without deleting or falsifying rival evidence.
7. **Exact binding is revisable without rewriting history.** Merge, split, rebind and supersession preserve the earlier basis.
8. **Source absence is not business deletion.** A CDC tombstone, missing spreadsheet row or API deletion describes source state unless the source is explicitly authoritative for the relevant deletion action.
9. **Schema/mapping revision is part of reproducibility.** A record captured under schema R and mapped by transform M must not later be silently interpreted using schema R+1 / transform M+1.
10. **Do not invent temporal precision.** Preserve source event/effective time when evidenced plus capture/record time. Missing business-valid time remains missing.
11. **Snapshots are legitimate observations.** If complete movements do not exist, represent an observed position rather than synthesize fake events.
12. **Authority is statement/action scoped.** Product registration, marketplace listing state, cost observation, price decision and fiscal authorization may have different authorities.
13. **Lineage-equivalent data is not independent evidence.** Copies/exports/materializations of one source must not be double-counted as separate business occurrences or independent corroboration.
14. **Extraction from text/images/LLMs produces proposals/evidence.** Human or agent interpretation can become an explicit decision; it must not silently become source-authored truth.
15. **Identity clusters are hypotheses.** Pairwise scores + clustering rules can create transitive groups whose semantic identity still needs domain constraints/review.
16. **Reprocessing is append/supersede, not historical mutation.** A newer model may improve a mapping; earlier decisions remain explainable under their original evidence/model revision.

## Primitive reduction result

Issue #45 explicitly attacks whether `Observation`, `Assertion`, `Fact`, `Binding`, `Qualification`, or `SourceRecord` must become generic engine primitives.
...
- Policies/authority rules for scoped operational qualification;
...
| [`ingest-contract.md`](ingest-contract.md) | Candidate lifecycle, relation/assurance/binding/qualification semantics, authority, schema drift and reprocessing |
...
qualified for operation X -> canonical truth everywhere
...
- scoped authority rules choosing which statements may drive an operation;
- materialized current projections for operations;
...
- **#40 commit semantics:** an exact-identity/adjudication or scoped qualification decision that affects business actions needs a state/revision contract; stale proposals must not commit against changed evidence unnoticed.
...
- **#42 authorization:** who may accept/rebind/merge/split exact identities, which statements may drive which operations, and what assurance an Action requires are separate from match confidence.
...
- **#49 observability:** must answer `which source evidence + mapping + relation/binding/qualification decision produced this current value or Action input?`
