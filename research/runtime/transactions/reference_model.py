...
class ImmutableReference(Basis):
    ...
    def check(self, world: World) -> tuple[bool, str]:  # noqa: ARG002 - world intentionally unused
...
class NoMutableDependency(Basis):
    def check(self, world: World) -> tuple[bool, str]:  # noqa: ARG002
...
class Approval:
    ...
    def permits(self, operation: "Operation") -> tuple[bool, str]:
        if not self.active:
            return False, "approval inactive/revoked by its own contract"
