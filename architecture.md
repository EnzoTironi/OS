...
**Ontology.** One Rust binary `zoen`: Connect API (`zoen serve`) and CLI (`zoen <noun> <verb>`). Meaning lives in committed canonical JSON. Publish is `DefinitionService.Publish`. The kernel is Rust. `zoen-core` holds types and laws with no IO. `zoen-engine` runs publish, propose, Cedar, and commit. `zoen-query` reads through DataFusion. `zoen-adapters` talks to Postgres 18, Rivet, and object storage. `zoen serve` is the composition root.

**Conversation.** Eve in `apps/conversation`. TypeScript. Durability is Eve, not Rivet. Isolate runs planted `zoen` and cannot commit.
...
Public protocol is Protobuf, Buf, and ConnectRPC. Definition identity is JCS plus SHA-256. Policy is Cedar. Untrusted code is Wasmtime. Ontology effects are Rivet behind `zoen-effect-dispatcher`. Conversation does not own Rivet.

Production is one machine. Image `deploy/fly/Dockerfile`. Postgres, Rivet, MinIO, the door, Eve, and zoend share the VM. Public HTTPS is zoend. The door and Eve bind loopback. WhatsApp inbound dest is Kapso at `/eve/v1/kapso`. Telegram inbound dest is Eve's first-class channel at `/eve/v1/telegram`. The product bar is Poke plus Palantir: Eve talks like a sharp friend; ontology verbs carry company-grade depth for every audience.
...
Callers do not pick Postgres versus Parquet, Cedar versus engine, or Rivet invocation details.
