...
Public HTTPS is `zoen.tironi.xyz` → zoend `:58701`. Postgres, Restate, and the Better Auth door stay up (`auto_stop_machines = off`). The door listens on `127.0.0.1:58704`. zoend forwards `/api/auth`, `/device`, and `/onboard/done` there. WhatsApp `/onboard/{token}` stays on zoend.
...
zoend boots on `ProcessAuth::SessionDoor` and `ZOEN_AUTH_DATABASE_URL`. Remint writes the opaque session token to `/data/zoen/agent.token`. zoend and Eve start after postgres and the auth loopback, not after JWKS. `ZOEN_BA_AGENT_PASSWORD` is a Fly secret, not committed.
